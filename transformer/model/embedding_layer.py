...
from transformer.utils import tpu as tpu_utils
...
class PositionEmbedding(tf.layers.Layer):
  """Calculates input embeddings and pre-softmax linear with shared weights."""
  ...
  def build(self, _):
    with tf.variable_scope("PositionEmbedding", reuse=tf.AUTO_REUSE):
      # Create and initialize weights. The random normal initializer was chosen
      # randomly, and works well.
      self.shared_weights = tf.get_variable(
          "weights", [self.vocab_size, self.hidden_size],
...
class SegmentEmbedding(tf.layers.Layer):
  ...
  def build(self, _):
    with tf.variable_scope("SegmentEmbedding", reuse=tf.AUTO_REUSE):
      # Create and initialize weights. The random normal initializer was chosen
      # randomly, and works well.
      self.shared_weights = tf.get_variable(
          "weights", [self.vocab_size, self.hidden_size],
