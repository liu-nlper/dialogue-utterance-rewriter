...
import translate
from transformer.utils import tpu as tpu_util
from transformer.utils import compute_bleu
...
from transformer.utils.params_mapping import init_model_with_bert
...
def model_fn(features, labels, mode, params):
  ...
  with tf.variable_scope("model"):
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
      ...
      return tf.estimator.EstimatorSpec(
          mode=mode, loss=loss, predictions={"predictions": logits},
          eval_metric_ops=metrics.get_eval_metrics(logits, targets, params))
    else:
...
def translate_and_compute_bleu(estimator, subtokenizer, bleu_source, bleu_ref,
  ...
  translate.translate_file(
      ...
      print_all_translations=False, vocab_file=vocab_file)
...
def evaluate_and_log_bleu(estimator, bleu_source, bleu_ref, vocab_file):
  ...
  uncased_score, cased_score = translate_and_compute_bleu(
      estimator, subtokenizer, bleu_source, bleu_ref, vocab_file)
...
def define_transformer_flags():
  ...
  flags.DEFINE_string(
      ...
      help=flags_core.help_wrap(
          ...
          "uncased BLEU score."))
  flags.DEFINE_string(
      name="data_train", default=None, help=flags_core.help_wrap("train file"))
  flags.DEFINE_string(
    name="data_dev", default=None, help=flags_core.help_wrap("dev file"))
  flags.DEFINE_string(
      ...
      help=flags_core.help_wrap(
          ...
          "the vocab file."))
  flags.DEFINE_string(
    name="bert_checkpoint", default=None,
    help=flags_core.help_wrap("Path to pre-trained BERT model"))
...
def construct_estimator(flags_obj, params, schedule_manager):
  ...
  if not params["use_tpu"]:
    ...
    run_config = run_config.replace(keep_checkpoint_max=100)
...
def run_transformer(flags_obj):
  ...
  params["max_length_source"] = \
    params["max_length_source"] or flags_obj.max_length_source
  params["max_length_target"] = \
    params["max_length_target"] or flags_obj.max_length_target
  ...
  schedule_manager = schedule.Manager(
      ...
      max_length=params["max_length_source"],
  ...
  params["bert_checkpoint"] = flags_obj.bert_checkpoint
