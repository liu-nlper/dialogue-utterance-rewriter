...
def _encode_and_add_eos(line, subtokenizer):
  ...
  CLS_ID = subtokenizer.subtoken_to_id_dict['[CLS]']
  SEP_ID = subtokenizer.subtoken_to_id_dict['[SEP]']

  query_ids = subtokenizer.encode(query)
  content_ids = subtokenizer.encode(content)

  token_ids = [CLS_ID] + query_ids + [SEP_ID] + content_ids + [tokenizer.EOS_ID]

  query_len = len(query_ids) + 2
  masks = [1] * query_len + [0] * (len(token_ids) - query_len)
...
def translate_file(
    ...
    print_all_translations=True, vocab_file=None):
  ...
  def input_fn():
    ...
    # TODO max_length_source, read from config file...
